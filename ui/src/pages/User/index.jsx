...
const User = () => {
  ...
  for (const option of selectBarOptions) {
    const param = option.queryParam;
    let search = "";
    if (param !== "") {
      queryParams.set("filter", param);
      search = queryParams.toString();
    }
    option.to = `${location.pathname}?${search}`;
  }
  ...
  const handleGiveSupporterBadge = async () => {
    try {
      if (hasSupporterBadge) {
        await mfetchjson(
          `/api/users/${user.username}/badges/supporter?byType=true`,
          {
            method: "DELETE",
          },
        );
  ...
  const renderBadges = (hideInMobile = true) => {
    ...
    return (
      <div
        className={`card card-sub page-user-modlist ${hideInMobile ? "is-no-m" : ""}`}
      >
        <div className="card-head">
        ...
        <header className="user-card card card-padding">
          <div className="user-card-top">
            <div className="user-card-top-left">
              ...
              />
              <h1
                className={`user-card-username ${hasSupporterBadge ? "is-supporter" : ""} ${user.deleted ? "is-red" : ""}`}
              >
                @{username}
                {user.isAdmin && (
                  <span className="user-card-is-admin">Admin</span>
                )}
              </h1>
            </div>
          ...
          <div className="user-card-joined">
            Joined on {dateString1(user.createdAt)}.
          </div>
          {user.deleted && (
            <div className="user-card-joined">
              Account deleted on {dateString1(user.deletedAt)}
            </div>
          )}
...
export function userHasSupporterBadge(user) {
  return (
    user &&
    user.badges.find((badge) => badge.type === "supporter") !== undefined
  );
}
