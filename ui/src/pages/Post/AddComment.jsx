...
import { ApiError, adjustTextareaHeight, mfetch } from "../../helper";
import {
  bannedFromAdded,
  loginPromptToggled,
  snackAlert,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        res = await mfetch(
          `/api/posts/${postId}/comments?userGroup=${userGroup}`,
          {
            method: "POST",
            body: JSON.stringify({
              parentCommentId,
    ...
    <div className={`post-comments-new ${editing ? "is-editing" : ""}`}>
      ...
      {(!main || (main && clicked)) && (
        <div className="post-comments-new-buttons">
          <Link
            className="button button-icon-simple"
            to="/markdown_guide"
            target="_blank"
          >
            <svg
