...
const Reports = ({ community }) => {
  ...
  const handleIgnore = async (report) => {
    try {
      await mfetchjson(
        `/api/communities/${community.id}/reports/${report.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  if (loading !== "loaded") {
    return <div className="modtools-content" />;
  }
  ...
  if (filter === "posts") {
    ...
    noPosts = details.noCommentReports;
  }
        ...
        {reports?.map((report) => {
          ...
          return (
            <div key={report.id} className="card card-padding card-report">
              ...
              {removed && (
                <div className="card-report-removed">
                  Removed by {removedUser}.
                </div>
              )}
              <div className="card-report-item">{item}</div>
              <div className="card-report-buttons">
                <button type="button" onClick={() => handleIgnore(report)}>
                  Ignore
                </button>
